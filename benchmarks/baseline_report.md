...
```bash
python scripts/run_benchmark.py --config benchmarks/benchmark_config.json --scope-folder "InBox" --query-text "Как снизить токены в пайплайне" --baseline benchmarks/baseline_metrics.json
```
...
Если baseline ещё не создан, сначала запустите без `--baseline`, затем скопируйте `metrics.json` в `benchmarks/baseline_metrics.json`.
